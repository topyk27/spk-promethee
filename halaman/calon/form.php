...
<?php foreach ($datas as $data): ?>
    <div class="row form-group">
        ...
        <div class="col-lg-9">
        	<input type="hidden" name="old_sub_id[]" value="<?php echo $data_calon['kriteria'][$data[0]['nama']]['id'] ?>">
            <?php if ($data[0]['nama_subkriteria']=='input'): ?>
                <input class="form-control" type="number" name="value[<?php echo $data[0]['subkriteria_id'] ?>]" placeholder="<?php echo $data[0]['nama']; ?>" required="" value="<?php if(isset($data_calon['kriteria'][$data[0]['nama']]['value'])) echo $data_calon['kriteria'][$data[0]['nama']]['value']; ?>">
                <input type="hidden" name="sub_id[<?php echo $data[0]['subkriteria_id'] ?>]" value="<?php echo $data[0]['subkriteria_id'] ?>">

            <?php else: ?>
                <select class="form-control" name="sub_id[<?php echo $data[0]['subkriteria_id'] ?>]" required>
